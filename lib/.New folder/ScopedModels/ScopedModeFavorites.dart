...
//-----------Get All Data by Token---------------//
  ...
  String _apiRemoveFavorites =
      ApiUtilities.baseApi + ApiUtilities.removeFavorites;
  ...
  initialStreamControllerFavorites() {
    streamControllerUserFavorites =
        StreamController<AllChaletsAndOffersModel>.broadcast();
  }

  //---------------------- Input and Output Stream Get All Chalet ----------------------//
  Stream<AllChaletsAndOffersModel> get outAllFavorites =>
      streamControllerUserFavorites.stream;
  Sink<AllChaletsAndOffersModel> get inPutAllFavorites =>
      streamControllerUserFavorites.sink;
  //----------Stop stream----------//
  ...
  Future getAllFavorites({BuildContext context}) async {
    ...
    final response = await http
        .get(_apiAllFavorites + token, headers: {"Accept": "application/json"});
  ...
  Future removeFavorites({BuildContext context, String chaletId}) async {
    ...
    var response = await http.post(_apiRemoveFavorites + token, body: {
    ...
    if (data['status'] == Utility.VERIFICATION_CODE_SUCCESS) {
      checkExistFavorites();
      Utility.showToast("تم الحذف من المفضله",
          context: context,
    ...
    ///========================================= Check the Status Response (FAILED)====================================//
      ...
      var error = AdvertiserNoChalets.fromJson(data);
      print(
          "============================ remove Favorites ${error.error} =================");
      stopLoadingHomePage();
      Utility.showToast("تم الحذف بالفعل",
          context: context,
          duration: Toast.LENGTH_LONG,
          chooseColor: 1,
          gravity: Toast.BOTTOM);
