...
class ScopedModelLogin extends Model {
  ...
  loginAndGetToken(String email, String password, BuildContext context) async {
    ...
    if (data['status'] == Utility.VERIFICATION_CODE_SUCCESS) {
      ...
      var response = await http.post(_apiUpdateInformation + token,
      ...
      if (role == "user") {
        ...
        pref.setString(Utility.ShARED_USER_NAME, body['data']['name']);
        pref.setString(Utility.ShARED_USER_PHONE, body['data']['phone'].toString());
      } else if (role == "advertiser") {
        saveTokenAndRoleId(roleId: 3, token: token);
        pref.setString(Utility.ShARED_Advertiser_EMAIL, email);
        pref.setString(Utility.ShARED_Advertiser_NAME, body['data']['name']);
        pref.setString(Utility.ShARED_Advertiser_PHONE, body['data']['phone'].toString());
        stopLoading();
        _openadvertiserPage(context);
      } else {
        print(
            "=====================Error in loginAndGetToken not user,not advertiser=================================");
      }
    }
    ...
    ///========================================= Check the Status Response (FAILED)====================================//
      ...
      print(
          "=======VERIFICATION_CODE_FAILED======== Done ${contentError} ==================");
      stopLoading();
